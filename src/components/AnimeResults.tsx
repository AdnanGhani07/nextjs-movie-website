...
export default async function AnimeResults() {
  ...
  if (data.length === 0) {
    data = await fetchAniListCatalog("ANIME", "SCORE_DESC", 1, 10);
  }

  if (!data || data.length === 0) {
    return (
      <div className="text-center py-10 glass-panel rounded-2xl">
        <p className="text-zinc-500 text-xs">Anime feed is currently unavailable.</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6">
      {data.map((item) => (
        <AnimeCard key={item.mal_id} item={item} type="anime" />
      ))}
    </div>
  );
}
